...
### 動作デモ（マウス操作およびボタン反応の可視化）
![動作デモ](screenshots/demo.gif)

### ゲーム画面のスクリーンショット
![ゲーム画面](screenshots/main_gameplay.png)
...
本プロジェクトには、GitHub Actions による自動化された CI/CD パイプラインが設定されています。設定内容は `.github/workflows/ci-cd.yml` で管理されており、`main` ブランチへのプッシュまたはプルリクエスト作成時に以下のステップが自動実行されます。

- **静的解析 (CI)**: ESLint (`npm run lint`) によるコード検証を行います。
- **自動テスト (CI)**: Playwright (`npm test`) を起動し、ロジック・描画・E2Eの全 46 テストを検証します。
- **自動デプロイ (CD)**: 静的解析およびテストのすべてのジョブが正常に完了した場合のみ、GitHub Pages へ最新のゲーム資産が自動デプロイされます。デプロイ方式には安全な `actions/deploy-pages` (Workflows) 方式を採用しています。
