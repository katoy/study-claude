...
- グローバルスコープの汚染防止と安全性の向上のため、コード全体を即時実行関数 (IIFE) 内に閉じ込める。テスト互換性を保つため、必要な変数や関数は `Object.defineProperty` (ゲッター/セッター) を介して `window` に明示的に露出させる

### セキュリティ

- **Content Security Policy (CSP)**: `index.html` 内の `<head>` に適切な CSP メタタグを維持し、自ドメインと許可された外部フォント（Google Fonts）以外の動作・読込を制限する
- **Service Worker キャッシュポリシー**:
  - `sw.js` でのキャッシュ処理は `GET` メソッドのみを対象とする
  - 不透明なレスポンス (Opaque Response) はキャッシュせず、ローカル配信用の静的アセット（`basic` レスポンス）のみを対象としてストレージ容量の圧迫を防ぐ
- **PWA 設定**: `manifest.json` の `start_url` は相対パス解決の曖昧さを避けるため、`"./index.html"` を使用する
