...
`index.html` ひとつで完結しているので、追加の依存関係やビルド手順はありません（※フォント読み込みのみ Google Fonts 外部 CDN に依存し、オフライン時はシステムフォントへフォールバックされます）。
...
- `SPACE` と `ハードドロップ` は最下段まで移動し、即座に固定されます
...
- ゲームオーバー中は操作できません
- `Enter` キーを押すか、タッチ画面の `リスタート` ボタンをタップすると最初からやり直せます
