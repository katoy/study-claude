...
describe('日付・時刻フォーマット・変換 (dateFormat.js)', () => {
  ...
  describe('UT-DAT-004: 日付差の計算 (daysBetween)', () => {
    it('同日の場合は 0 を返すこと', () => {
      // JST での日付比較なので、同じ JST 日付なら差分は 0
      expect(daysBetween('2026-07-24', '2026-07-24')).toBe(0);
      // 両方が JST 2026-07-24 内：2026-07-23T15:00:00.000Z (JST 2026-07-24 00:00) と
      // 2026-07-24T14:59:59.000Z (JST 2026-07-24 23:59)
      expect(daysBetween('2026-07-23T15:00:00.000Z', '2026-07-24T14:59:59.000Z')).toBe(0);
    });

    it('翌日の場合は 1 を返すこと', () => {
      expect(daysBetween('2026-07-24', '2026-07-25')).toBe(1);
      // JST 2026-07-24 と JST 2026-07-25 の差分
    ...
    it('昨日の場合は -1 を返すこと', () => {
      ...
      // JST 2026-07-24 と JST 2026-07-23 の差分
      expect(daysBetween('2026-07-24T14:59:59.000Z', '2026-07-23T00:00:00.000Z')).toBe(-1);
    });

    it('タイムゾーン境界や夏時間などの微細なミリ秒差に左右されず、正しく計算されること', () => {
      // JST 2026-07-24 と JST 2026-07-25 の差分（JST ベース計算）
      // JST 2026-07-24 00:00:00 (UTC 2026-07-23 15:00:00) と
      // JST 2026-07-25 23:59:59 (UTC 2026-07-25 14:59:59)
      expect(daysBetween('2026-07-23T15:00:00.000Z', '2026-07-25T14:59:59.000Z')).toBe(1);

      // 3日分の差分を検証
      // JST 2026-07-24 と JST 2026-07-27 の差分
      expect(daysBetween('2026-07-23T15:00:00.000Z', '2026-07-27T14:59:59.000Z')).toBe(3);
    });
  });
});
