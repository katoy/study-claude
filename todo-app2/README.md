...
## 🔒 セキュリティ対策と監査（Security Review）

本アプリケーションは、安全で堅牢なシングルページアプリケーションを維持するため、静的解析ツールおよび依存関係の監査を導入しています。
...
* **静的コード解析によるサニタイズ漏れの防止 (`eslint-plugin-no-unsanitized`)**:
  * ESLint に `no-unsanitized` プラグインを導入しており、サニタイズ処理を経由せずに `.innerHTML` などのプロパティへの代入が行われた場合、ビルド・コミット時に自動で検知してエラーを発生させます。
* **防御的データパース**:
  * `localStorage` からのデータ復元時に、型チェックおよびUUIDの厳格な検証を行い、不正なデータが保存されていてもアプリケーションのクラッシュやデータ汚染を防ぎます。

### 2. セキュリティ検査の実行方法 (利用方法)

セキュリティレビュアーとしてコードや依存関係の安全性を確認するには、以下のコマンドを実行します。
