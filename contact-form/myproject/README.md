...
- ステータス絞り込み（新規・対応中・解決済み）および絞り込み条件に連動するリアルタイム件数バッジ表示
...
│   │   │   └── counted-field.blade.php # 文字数表示・エラー関連付けを共通化
│   │   └── vendor/pagination/
│   │       └── tailwind.blade.php      # 日本語・アクセシブルなページネーション
│   ├── css/
...
- **ContactFormTest**: 公開フォームの3ステップフロー
  - フォーム表示（全入力項目の文字数カウンター、絵文字の文字数整合、超過警告、ARIA属性を含む）
...
- **Admin/ContactControllerTest**: 管理画面の認証・CRUD・ソート・絞り込み
  - 未認証・非管理者のアクセス拒否
  - 一覧表示・ページネーション（絞り込み・ソート・ページ・表示件数の保持）
  - ナビゲーション、絞り込み、ページネーションのアクセシビリティと日本語表示
  - モバイル幅で絞り込みを閉じる初期状態
  - **ソート機能**: 登録日時（昇順/降順）、ステータス、名前
  - **絞り込み機能**: ステータス、キーワード検索（名前・メール・件名）、日付範囲
  - **エッジケース**: 不正な値のフォールバック、日付逆転の入れ替え、タイムゾーン境界
  - **Ajax 対応**: 非同期リクエストのビュー切り替え
  - 詳細表示
  - ステータス更新
  - 更新成功メッセージ
  - 日本時間での日時表示（JST/UTC 変換検証）
  - 不正な値の検証（エラー画面なし、デフォルト値へ）
...
- **総テスト数**: 93 個（280アサーション）
- **テストカバレッジ**: 100.0%
- **すべてPASS**: ✓ 100% (0 failures)
...
php artisan test --filter=Admin\\ContactControllerTest
