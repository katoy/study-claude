...
- [MCP (Model Context Protocol) サーバー](#mcp-model-context-protocol-サーバー)
- [ディレクトリ構造](#ディレクトリ構造)
- [開発ガイド](#開発ガイド)
...
本プロジェクトでは、AI エージェント（Claude 等）が開発やデバッグをより円滑に進められるよう、以下の Model Context Protocol (MCP) サーバーが `.mcp.json` に定義されています。

### 1. Playwright MCP (`playwright-mcp`)
- **概要**: ブラウザ自動化ツール Playwright を AI エージェントから直接操作できるようにする MCP サーバーです。
- **用途**: 
  - ブラウザをシミュレートした UI の自動動作テスト
  - 操作デモ GIF や画面スクリーンショットの自動撮影・確認
  - ブラウザコンソールログのデバッグ

### 2. Claude in Chrome (`claude-in-chrome`) / `use-chrome`
- **概要**: ユーザーが普段利用している Google Chrome ブラウザと AI エージェントを安全に接続・操作するための MCP サーバーです。
- **用途**:
  - Chrome ブラウザインスタンスを利用したアプリケーションの挙動検証
  - セッション情報や既存の認証状況を引き継いだ状態での UI デバッグ

### 設定の有効化方法
本プロジェクトのルートにある `.mcp.json` および `.claude/settings.json` にて、上記 MCP サーバーが有効化されています。Claude Code 等の対応クライアントを起動すると、自動的にこれらのツールがエージェントに読み込まれ、ブラウザ操作が必要なタスク（画面崩れのチェック、スクリーンショットの撮影など）を自律的に実行可能になります。
