...
- **Database**: SQLite (`database/database.sqlite`)。正式な本番出荷構成でも、永続ローカルファイルシステム上のSQLiteを使用する
...
### コード品質・スタイル
- **一括整形・静的解析**: `npm run lint` (Pint, remark-cli, htmlhint, eslintを一括実行)
...
- **静的スクリーンショット (JPEG)**:
  - 表示領域 `1280x800`、device scale factor `2`、ロケール `ja-JP`、ダークテーマに統一し、**フルページ (`full_page=True`)** で撮影する。
...
- CSVエクスポート機能:
  - 絞り込み条件に連動したCSVデータのストリーミング出力（BOM付きUTF-8、Excel対応）
  - 件数制限: 最大 10,000 件（超過時は422エラー画面に遷移。`.env` の `CONTACT_EXPORT_LIMIT` で変更可能）
  - レート制限: 1分間あたり2回まで
  - CSVインジェクション対策: 特定記号（`=`, `+`, `-`, `@`, `＝`, `＋`, `－`, `＠`）で始まる値に `\t` を付与
  - 監査ログ記録: 実行時に管理者ID、検索・ソートフィルター条件、エクスポート件数を `Log::info` に出力
...
- PSR-12に準拠
- コメントは日本語
- コントローラーの肥大化を防ぐため、クエリパラメータのパース・正規化には専用のDTO（`App\DTO`）を使用し、ビジネスロジック（DBトランザクション・ロギング等）はサービス層（`App\Services`）へ抽出・委譲する
...
## デプロイ方針
- 正式な本番出荷構成は、1台のホスト、永続ローカルファイルシステム、SQLiteとする。同一ホスト上の複数PHP-FPM workerは同じSQLiteファイルを共有する。
- Cloud Run等の複数インスタンス、ネットワークファイルシステム、Cloud SQL／Supabase等の外部DBは、現在の正式な出荷構成に含めない。
- GitHub Pagesは静的ホスティングのため、LaravelのPHP／DB処理を含む本アプリケーションの出荷先にしない。
- 別構成へ移行する場合は [ADR-001](docs/decisions/001-single-host-sqlite-deployment.md) を新しいADRで置き換え、DB整合性、排他制御、ファイル配信、バックアップを再設計・再検証する。
