...
Alpine.data("contactFilters", (initial) => ({
  status: Array.isArray(initial.status)
    ? initial.status
    : initial.status
      ? String(initial.status).split(",")
      : [],
  ...
  loadHistory() {
    try {
      this.keywordHistory = JSON.parse(
        localStorage.getItem("contact_keyword_history") || "[]"
      );
      this.bodyKeywordHistory = JSON.parse(
        localStorage.getItem("contact_body_keyword_history") || "[]"
      );
      this.dateHistory = JSON.parse(
        localStorage.getItem("contact_date_history") || "[]"
      );
  ...
  saveCurrentKeywordsToHistory() {
    if (this.keyword.trim()) {
      ...
      this.keywordHistory = [
        val,
        ...this.keywordHistory.filter((item) => item !== val),
      ].slice(0, 10);
      try {
        localStorage.setItem(
          "contact_keyword_history",
    ...
    if (this.bodyKeyword.trim()) {
      ...
      this.bodyKeywordHistory = [
        val,
        ...this.bodyKeywordHistory.filter((item) => item !== val),
      ].slice(0, 10);
      try {
        localStorage.setItem(
          "contact_body_keyword_history",
          JSON.stringify(this.bodyKeywordHistory)
        );
      } catch {
        // localStorage may be unavailable in privacy-restricted browsers.
      }
    ...
    if (this.dateFrom || this.dateTo) {
      ...
      try {
        localStorage.setItem(
          "contact_date_history",
          JSON.stringify(this.dateHistory)
        );
      } catch {
        // localStorage may be unavailable in privacy-restricted browsers.
      }
  ...
  removeBodyKeywordHistoryItem(val) {
    this.bodyKeywordHistory = this.bodyKeywordHistory.filter(
      (item) => item !== val
    );
    try {
      localStorage.setItem(
  ...
  removeDateHistoryItem(item) {
    this.dateHistory = this.dateHistory.filter(
  ...
  buildQueryString() {
    ...
    if (this.sort && this.sort !== "created_at-desc")
      params.set("sort", this.sort);
    if (this.perPage && Number(this.perPage) !== 20)
  ...
  async fetchResults(url = null, pushState = true) {
    this.saveCurrentKeywordsToHistory();
    const target =
      url ?? `${window.location.pathname}?${this.buildQueryString()}`;
    this.loading = true;
