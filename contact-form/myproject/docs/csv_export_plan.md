...
13. `test_export_csv_header_labels_localized_in_english` → `en.json` に英訳を追加し、英語ロケールでヘッダーが正しく翻訳されることを確認
...
public function export(Request $request): StreamedResponse
{
    ...
    return response()->streamDownload(function () use ($contacts) {
        ...
        fputcsv(
            $handle,
            ['ID', __('名前'), __('メールアドレス'), __('件名'), __('本文'), __('ステータス'), __('登録日時')],
            separator: ',',
            enclosure: '"',
            escape: '',
            eol: "\r\n",
...
既存の `index()` メソッド（54〜80行目）の `view()` 呼び出し直前に、正規化済みの `$exportQuery` を生成してビューに渡す。`$exportQuery` には、正規化済みの `$filters` から、ユーザーが指定した条件（ステータス、キーワード、日付）と選択したソート順を含める。ページネーション（`page`/`per_page`）は含めない。

**`sort` パラメータについて**：`$filters['sort']` は常にデフォルト値または正規化値を持つため、`$exportQuery` に常に渡される（意図的）。これにより、一覧表示とエクスポート時に同じソート順を保証する。
...
16. `test_export_csv_executes_single_contact_select_query` — 501件以上でもクエリが1回だけ実行され、`lazy()` による複数回の分割クエリになっていないことを確認
